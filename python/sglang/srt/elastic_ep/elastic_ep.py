...
class ElasticEPStateManager:
    ...
    def init(cls, server_args: ServerArgs):
        ...
        if server_args.elastic_ep_backend is not None:
            ...
            cls._instance.ep_join_rank_offset = server_args.ep_join_rank_offset
...
def elastic_world_logits_gather_enabled() -> bool:
    """Whether logits gather must run over Mooncake WORLD instead of the launch-time TP group.

    True after a scale-up has expanded ``effective_ep_size`` to ``max_ep_size``,
    when ``max_ep_size`` strictly exceeds the launch-time TP group size. The
    launch-time TP group does not span joiner ranks post-scale, so logits
    gathered over it would miss joiner-owned vocab shards. WORLD spans both
    halves and is the only group ``recover_ranks()`` updates.

    Symmetric across primary and joiner: both derive ``max_ep_size`` and
    ``tp_size`` identically from CLI args, so the gate flips on across all
    ranks at the same point. Cold-start runs where ``tp_size == max_ep_size``
    (no scale event) take the upstream NCCL all-gather path.
    """
    ...
    if max_ep_size <= 0 or max_ep_size <= sa.tp_size:
        return False
    return inst.effective_ep_size >= max_ep_size
