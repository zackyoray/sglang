...
class LogitsMetadata:
    ...
    def compute_dp_attention_metadata(self):
        ...
        max_ep_size = getattr(sa, "max_ep_size", None) or 0
        use_full_token_counts = (
            inst is not None
            and max_ep_size > 0
            and inst.effective_ep_size >= max_ep_size
            and max_ep_size > sa.tp_size
        )
        tokens_gpu = (
...
class LogitsProcessor(nn.Module):
    ...
    def _elastic_world_logits_gather_enabled(self) -> bool:
        """True once an elastic scale event has grown the live world.

        The default `tensor_model_parallel_all_gather` would gather logits
        across the launch-time NCCL TP group, which is disjoint from the
        joiner's TP group post-scale and produces corrupt joiner-owned
        rows. `_gather_logits_via_world_all_reduce` writes per-owner
        slices into a zero-initialised buffer and SUM-reduces over the
        Mooncake WORLD PG, which spans both halves.

        Symmetric on primary and joiner: both compare ``effective_ep_size``
        to ``server_args.tp_size`` (the launch-time NCCL TP group size).
        On the joiner, ``original_ep_size`` already equals the post-scale
        world (the joiner is launched into the grown world), so a check
        against ``original_ep_size`` would be False on the joiner half
        and the joiner would fall back to the broken NCCL all-gather.
        """
        inst = ElasticEPStateManager.instance()
        if inst is None:
            return False
        ...
        if max_ep_size <= 0:
            return False
        # Off until we are fully scaled to max_ep_size on every rank, and
        # only when max_ep_size actually exceeds the launch-time TP group.
        return (
            inst.effective_ep_size >= max_ep_size
            and max_ep_size > sa.tp_size
        )
    ...
    def _get_pruned_states(
        ...
        if (
            ...
            if (
                self._elastic_world_logits_gather_enabled()
