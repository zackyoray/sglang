...
| `--ep-join-mode` | Join mode for elastic EP. `recover` rejoins an existing slot after a fault. `scale` joins as a brand-new rank beyond the original group size. | `None` | `scale`, `recover` |
